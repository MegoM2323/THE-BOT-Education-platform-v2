...
// createTestTemplate создает шаблон урока с указанными уроками
func createTestTemplate(t *testing.T, db *sqlx.DB, adminID uuid.UUID, name string, lessons []*models.CreateTemplateLessonRequest) *models.LessonTemplate {
	...
	// Добавляем уроки в шаблон
	for _, lesson := range lessons {
		// Apply defaults to ensure EndTime is set
		lesson.ApplyDefaults()

		lessonEntryID := uuid.New()

		insertQuery := `
			INSERT INTO template_lessons (id, template_id, day_of_week, start_time, end_time, teacher_id, max_students, color, lesson_type, created_at, updated_at)
		...
		require.NoError(t, err, "Failed to create template lesson entry")

		// Добавляем студентов к уроку
		for _, studentID := range lesson.StudentIDs {
			studentEntryID := uuid.New()
			studentQuery := `
				INSERT INTO template_lesson_students (id, template_lesson_id, student_id, created_at)
				VALUES ($1, $2, $3, $4)
			`

			_, err := db.ExecContext(ctx, studentQuery,
				studentEntryID,
				lessonEntryID,
				studentID,
				time.Now(),
			)
			require.NoError(t, err, "Failed to add student to template lesson")
		}
	}

	return &models.LessonTemplate{
		ID:      templateID,
		AdminID: adminID,
		Name:    name,
	}
